...
export const banService = {
  ...
  async create(args: CreateBanArgs): Promise<BannedIp | null> {
    ...
    if (existing) {
      // Refresh existing: extend banned_until if new window is more restrictive (null =
      // permanent beats any timestamp).
      const currentUntil = existing.banned_until ? new Date(existing.banned_until as Date) : null;
      let newUntil = currentUntil;
      if (bannedUntil == null) newUntil = null; // permanent absorbs
      ...
      const [updated] = await db('banned_ips').where({ id: existing.id }).update({
        ...
        last_hit_at: now,
        is_active: true, // re-arm if previously soft-deleted
        updated_at: now,
      }).returning('*');
      return rowToBan(updated);
    }
